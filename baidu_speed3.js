...
if ($.isNode()) {
  if (process.env.BAIDU_COOKIE3 && process.env.BAIDU_COOKIE3.indexOf('&') > -1) {
  StartBody = process.env.BAIDU_COOKIE3.split('&');
  }
 if (process.env.BAIDU_COOKIE3 && process.env.BAIDU_COOKIE3.indexOf('\n') > -1) {
  BDCookie3 = process.env.BAIDU_COOKIE3.split('\n');
  } else {
  BDCookie3 = process.env.BAIDU_COOKIE3.split()
  }
  Object.keys(BDCookie3).forEach((item) => {
        if (BDCookie3[item]) {
          CookieArr.push(BDCookie3[item])
        } 
    })
} else {
