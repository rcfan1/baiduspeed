...
if ($.isNode()) {
  if (process.env.BAIDU_COOKIE4 && process.env.BAIDU_COOKIE4.indexOf('&') > -1) {
  StartBody = process.env.BAIDU_COOKIE4.split('&');
  }
 if (process.env.BAIDU_COOKIE4 && process.env.BAIDU_COOKIE4.indexOf('\n') > -1) {
  BDCookie4 = process.env.BAIDU_COOKIE4.split('\n');
  } else {
  BDCookie4 = process.env.BAIDU_COOKIE4.split()
  }
  Object.keys(BDCookie4).forEach((item) => {
        if (BDCookie4[item]) {
          CookieArr.push(BDCookie4[item])
        } 
    })
} else {
