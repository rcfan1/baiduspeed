...
if ($.isNode()) {
  ...
  StartBody = process.env.BAIDU_COOKIE2.split('&');
  }
 if (process.env.BAIDU_COOKIE2 && process.env.BAIDU_COOKIE2.indexOf('\n') > -1) {
  BDCookie2 = process.env.BAIDU_COOKIE2.split('\n');
  } else {
  BDCookie2 = process.env.BAIDU_COOKIE2.split()
  }
  Object.keys(BDCookie2).forEach((item) => {
        if (BDCookie2[item]) {
          CookieArr.push(BDCookie2[item])
        } 
    })
} else {
